...
export class PeriodService {
  ...
  async create(createPeriodDto: CreatePeriodDto): Promise<Period> {
    ...
    const newPeriod = this.periodRepository.create({
      ...
      accp_start_date: new Date(start_date),
      accp_end_date: new Date(end_date),
      accp_status: 'O', // Open
  ...
  async createYearPeriods(createYearPeriodsDto: CreateYearPeriodsDto): Promise<Period[]> {
    ...
    try {
      ...
      // Períodos mensuales (1-12)
      for (let i = 1; i <= 12; i++) {
        const startDate = new Date(year, i - 1, 1);
        const endDate = new Date(year, i, 0); // Último día del mes

        const period = queryRunner.manager.create(Period, {
          accp_cmpy: cmpy,
          accp_year: year,
          accp_per: i,
          accp_description: this.getMonthName(i),
          accp_start_date: startDate,
          accp_end_date: endDate,
          accp_status: 'O', // Open
          accp_is_closing_period: false,
          accp_creation_by: creation_by,
        });
      ...
      // Período de cierre (13)
      const closingPeriod = queryRunner.manager.create(Period, {
        accp_cmpy: cmpy,
        accp_year: year,
        accp_per: 13,
        accp_description: 'Cierre Anual',
        accp_start_date: new Date(year, 11, 31), // 31 de diciembre
        accp_end_date: new Date(year, 11, 31), // 31 de diciembre
        accp_status: 'O', // Open
        accp_is_closing_period: true,
        accp_creation_by: creation_by,
      });

      await queryRunner.manager.save(closingPeriod);
      periods.push(closingPeriod);
  ...
  private async executeYearClosing(queryRunner: any, cmpy: string, year: number, userId: string): Promise<void> {
    ...
    // 3. Crear o verificar que exista el año siguiente
    const nextYear = year + 1;
    let nextYearPeriod = await queryRunner.manager.findOne(Period, {
      where: {
        accp_cmpy: cmpy,
        accp_year: nextYear,
        accp_per: 1, // Primer período del año siguiente
      },
    });

    if (!nextYearPeriod) {
      // Crear períodos para el año siguiente
      const startDate = new Date(nextYear, 0, 1); // 1 de enero
      const endDate = new Date(nextYear, 0, 31); // 31 de enero

      nextYearPeriod = queryRunner.manager.create(Period, {
        accp_cmpy: cmpy,
        accp_year: nextYear,
        accp_per: 1,
        accp_description: 'Enero',
        accp_start_date: startDate,
        accp_end_date: endDate,
        accp_status: 'O', // Open
        accp_is_closing_period: false,
      ...
      await queryRunner.manager.save(nextYearPeriod);
    }
