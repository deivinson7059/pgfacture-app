...
@Index('idx_accounting_period_cmpy_year_status', ['accp_status', 'accp_cmpy', 'accp_year'])
...
export class Period {
    ...
    @Expose({ name: 'status' })
    accp_status: string; // 'O' = Open (Abierto), 'C' = Closed (Cerrado), 'B' = Blocked (Bloqueado), 'P' = Processing (En proceso)
