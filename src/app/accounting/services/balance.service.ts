...
export class BalanceService {
    ...
    async generarBalancePorRangoFechas(
        ...
        try {
            // 1. Eliminar cualquier balance existente con el tipo, compañía y fechas
            await queryRunner.manager.delete(BalanceDetail, {
                acbd_cmpy: cmpy,
                acbd_type: type,
                acbd_year: 0, // Usamos año 0 para balances por rango de fechas
                acbd_per: 0, // Usamos período 0 para balances por rango de fechas
                acbd_date_ini: startDate,
                acbd_date_end: endDate,
            });
            ...
            let balance = await this.balanceRepository.findOne({
                where: {
                    accb_cmpy: cmpy,
                    accb_year: 0, // Usamos año 0 para balances por rango de fechas
                    accb_per: 0, // Usamos período 0 para balances por rango de fechas
                    accb_type: type,
                    accb_date_ini: startDate,
                    accb_date_end: endDate,
                },
            ...
            // 4. Obtener movimientos del libro mayor para el rango de fechas especificado
            const ledgerEntries = await this.ledgerRepository.find({
                where: {
                    accl_cmpy: cmpy,
                    accl_ware: ware,
            ...
            for (const account of allAccounts) {
                ...
                if (isClass || isParent || hasMovements) {
                    ...
                    const balanceDetail = queryRunner.manager.create(BalanceDetail, {
                        acbd_cmpy: cmpy,
                        acbd_year: 0, // Año 0 para balances por rango
                        acbd_per: 0, // Período 0 para balances por rango
                        acbd_type: type,
                        acbd_date_ini: startDate,
                        acbd_date_end: endDate,
                        acbd_account: accountId,
    ...
    async obtenerBalancePorRangoFechas(
        cmpy: string,
        startDate: Date,
        endDate: Date,
        type: string
